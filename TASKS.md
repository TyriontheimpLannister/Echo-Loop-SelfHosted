...
> 最后更新：2026-07-31（发布 HomeSchooling 任务多选版 1.0.42）
...
- [x] 2026-08-01：完成句子级 AI 追问；根据真机反馈把入口补到解析结果下方，发布 1.0.45；后端真实契约 5/5，待真机交互验收。
- [x] 2026-08-01：修复 HomeSchooling 整篇导入的伪 2 秒字幕边界；现按实际句子片段真实时长生成一个音频的逐句 SRT，并已发布 1.0.43。
- [x] 2026-07-31：移除可见的旧 HomeSchooling JSON 文件包入口；在线接收改为密码后展示当前孩子全部任务，支持多选、整篇/逐句模式、顺序导入及逐项结果反馈；已发布 1.0.42。
- [x] 2026-07-31：修复 HomeSchooling 在线拉取按缺失 `child_slug` 筛选导致无任务；改用 `child_id`，增加即时/持久反馈，并发布 1.0.41。
- [x] 2026-07-31：移除自建后端不支持的官方合集、AI 聊天入口和远程配置轮询；第三方分析改为仅本地日志并移除 Firebase/PostHog/友盟 SDK。
- [x] 2026-07-31：完成双账号/HomeSchooling 映射收尾，修复 APK 更新地址注入并发布 1.0.39；刷新 Graphify code-only 图谱。

- [x] 2026-07-28 23:40：将“发送到 HomeSchooling 听写”从 Android 系统分享改为家长密码鉴权、选择孩子后直接导入 HomeSchooling。
- [x] 2026-07-28 23:22：修复 HomeSchooling 逐句导出误用 FFmpeg `min` 未包含的 `libmp3lame`；改用内置 AAC/M4A，发布带版本号 APK 1.0.33。
